...
1. In words, explain the difference between a `BEFORE` and an `AFTER` trigger, and give a realistic use case for each.
2. Write a PostgreSQL trigger that prevents any book from being deleted if its price is above $100 (hint: use a `BEFORE DELETE` trigger and `RAISE EXCEPTION`).
3. What's available inside an `UPDATE` trigger that isn't available inside an `INSERT` trigger?
4. Why might overusing triggers make a codebase harder to debug?

### Answers

```sql
-- 1
-- BEFORE triggers run before the change is applied, and are typically
-- used to validate or modify incoming data (e.g., rejecting a negative
-- price). AFTER triggers run once the change has already taken effect,
-- and are typically used for logging or updating related data (e.g.,
-- writing a price-change history row).

-- 2
CREATE FUNCTION prevent_expensive_delete() RETURNS TRIGGER AS $$
BEGIN
    IF OLD.price > 100 THEN
        RAISE EXCEPTION 'Cannot delete books priced above $100';
    END IF;
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_prevent_expensive_delete
BEFORE DELETE ON books
FOR EACH ROW
EXECUTE FUNCTION prevent_expensive_delete();

-- 3
-- OLD (the pre-change row values) — an INSERT trigger has no "before"
-- state, so only NEW is available there.

-- 4
-- Triggers fire invisibly in response to ordinary statements, without
-- appearing anywhere in application code, so unexpected side effects
-- ("why did this other table change?") can be difficult to trace back
-- to their cause.
```
