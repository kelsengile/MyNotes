...
**Example:** to read from a disk, the OS might write a target sector number into one controller register, set an "operation" register to "read," then poll or wait for interrupt on a "status" register — all without the rest of the OS needing to know these register addresses at all, since a device driver (14.4) handles that translation.
...
|---|---|---|
| CPU behavior while waiting | Busy-loops, wastes cycles | Free to do other work |
| Best for | Very fast devices | Slow devices (disks, keyboards, network) |
| Complexity | Simple | Requires interrupt handling infrastructure |
...
**Example:** copying a 500MB file from disk to memory without DMA would require the CPU to execute a copy instruction for every single chunk of data — with DMA, the CPU issues one setup instruction, then continues running other programs while the DMA controller streams all 500MB directly into RAM, notifying the CPU only once at the very end.
...
```
Application
    |  "write these bytes to this file"
Generic File System Interface
    |
Device Driver (specific to this exact SSD model)
    |  precise register operations for this hardware
SSD Controller
```

A different SSD from a different manufacturer swaps in a different driver underneath the same generic interface — the application and file system above never need to change.
