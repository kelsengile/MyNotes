...
> 💡 **Analogy:** A classifier is like an art critic — it looks at a painting and tells you what it depicts. A generative model is like the painter — it creates the painting from nothing but an idea.
...
```mermaid
flowchart LR
    A[Pre-training on huge text corpus] --> B[Instruction Tuning]
    B --> C[Alignment / RLHF]
    C --> D[Deployed Chat Assistant]
```
...
> 💡 **Analogy:** Prompting is like giving a very knowledgeable new employee detailed instructions for a one-off task. Fine-tuning is like sending that same employee through weeks of specialized training so a whole category of tasks becomes second nature.

| | Prompting | Fine-Tuning |
|---|---|---|
| Changes the model? | No | Yes |
| Cost | Low | Higher |
| Best for | One-off or varied tasks | Consistent, specialized behavior |
...
**🔍 Quick Example:** Ask an LLM for a legal citation and it might confidently produce a case name, year, and court that sound completely real — but don't actually exist. It's not "lying"; it's generating the most statistically plausible-sounding text, with no fact-checking step built in.
