...
```
Classification:  "This image contains a dog."           (whole image → 1 label)

Object Detection:  ┌────────────┐
                    │  [DOG] 🐕  │  <- bounding box + label
                    └────────────┘
                          🌳            🚗
                         (no box)     [CAR]
```

> 💡 **Analogy:** Classification answers "what's in this photo?" Object detection answers "what's in this photo, and exactly where?" — like the difference between saying "there are people in this photo" versus circling each one and labeling them.
...
```mermaid
flowchart LR
    A[Random Noise] --> B[Step 1: Vague Shapes] --> C[Step 2: Rough Details] --> D[Final: Coherent Image]
```

> 💡 **Analogy:** Diffusion is like a sculptor starting with a rough, shapeless block of static and slowly chiseling away the noise — guided by your text prompt — until a clear image emerges.
