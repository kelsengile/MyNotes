...
A typical project folder looks like this from the outside:

```
my-project/
├── .git/          <- the entire history lives here
├── src/
│   └── app.py
├── README.md
└── .gitignore
...
Everything outside `.git/` is just the current snapshot — delete `src/app.py` by accident, and as long as `.git/` is intact, every past version of it is still recoverable.
...
A single commit, as Git records it, looks something like this:

```
commit 7e8f9a0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a
Author: Ada Lovelace <ada@example.com>
Date:   Tue Sep 8 14:22:03 2026 -0700

    Add discount code support to checkout

    Applies a percentage discount before tax is calculated,
    and validates the code against the promotions table.
```

The chain of commits underneath forms a graph — each arrow below points to the previous commit:
...
Working Directory   --git add-->   Staging Area   --git commit-->   History
  (edit files)                     (chosen changes)                 (permanent record)
```

For example, if you've edited both `app.py` and `notes.txt`, but only want to commit the `app.py` change:

```bash
git add app.py        # only app.py moves into the staging area
git commit -m "Fix off-by-one error in pagination"
# notes.txt remains an uncommitted, unstaged edit for later
```
...
A small diff, as `git diff` would display it:
...
Lines starting with `-` were removed, lines starting with `+` were added — everything else stayed the same.
