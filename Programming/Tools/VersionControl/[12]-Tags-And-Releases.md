...
```bash
git tag v1.2.0-lite                              # lightweight
git tag -a v1.2.0 -m "Release 1.2.0"             # annotated, recommended for real releases
...
|---|---|---|
| Breaking change to the API | Removed a public function | `2.4.1` → `3.0.0` |
| New backward-compatible feature | Added an optional parameter | `2.4.1` → `2.5.0` |
| Backward-compatible bug fix | Fixed a rounding error | `2.4.1` → `2.4.2` |
...
A short set of release notes might look like:
...
### Fixed
- Rounding error in tax calculation on international orders
```
