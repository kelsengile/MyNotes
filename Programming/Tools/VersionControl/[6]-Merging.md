...
A ── B ── C ── D  (main, after)  A ── B ── E ── F  (main)
     └── C ── D  (feature)            └── C ── D ─┘
                                                    \
                                                     M  (new merge commit)
```
...
Everything between `<<<<<<< HEAD` and `=======` is your current branch's version; everything between `=======` and `>>>>>>> feature/dark-mode` is the incoming branch's version.
...
Resolving the example above by keeping the dark-mode color, then finishing the merge:

```python
button_color = "dark_gray"   # markers removed, final decision made
```
